 ...
 */
const filterByProperties = (tag: string, audience: string, trainings: Types.Training[]): Types.Training[] => {
  if (tag === 'all' && audience === 'all') return trainings;

  let filtered = trainings;
  if (audience !== 'all') {
    filtered = filtered.filter(
      t => t.audiences?.length > 0 && t.audiences.findIndex(ad => ad?.toLowerCase() === audience.toLowerCase()) > -1,
    );
  }
  if (tag !== 'all') {
    filtered = filtered.filter(
      t => t.tags?.length > 0 && t.tags.findIndex(ta => ta?.toLowerCase() === tag.toLowerCase()) > -1,
    );
  }
  return filtered;
};
