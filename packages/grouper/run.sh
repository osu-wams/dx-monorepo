...
if [ "$1" == "docker" ] || [ "$1" == "" ]
then
  docker build -t deno-grouper -q .
  if [ "$2" == "run" ] || [ "$2" == "" ]
  then
    docker run -it --init -v $PWD/log:/app/log --env-file .env deno-grouper
  elif [ "$2" == "shell" ]
  then
    docker run -it --init -v $PWD/log:/app/log --env-file .env --entrypoint sh deno-grouper
  fi
elif [ "$1" == "local" ]
then
  deno run --allow-write --allow-net --allow-env test.ts
fi
