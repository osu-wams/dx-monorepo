...
import { Resources } from '@osu-wams/utils';
...
const filteredResourcesByUserAffiliation = selector<Types.Resource[]>({
  ...
  get: ({ get }) => {
    ...
    return resources.data.filter(r => Resources.checkAffiliation(user.data, r));
  },
});
...
const filteredResourcesByCategory = selector<Types.Resource[]>({
  ...
  get: ({ get }) => {
    ...
    return Resources.filterByCategory(user.data, selectedCategory, resources);
  },
});
...
const filteredResourcesBySearch = selector<Types.Resource[]>({
  ...
  get: ({ get }) => {
    ...
    const foundIds = found.filter(i => Boolean(i.item.attr.resource)).map(i => i.item.attr.resource?.id);
