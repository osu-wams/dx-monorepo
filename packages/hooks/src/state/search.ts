...
import { Canvas, Course, Helpers, Routes } from '@osu-wams/utils';
    ...
    const all = events
      .map(({ audience, items: { data } }) =>
        data.map(event => ({
          ...
          title: event.title,
          subText: {
            value: Helpers.format(event.date, 'EEEE, MMMM d, yyyy'),
          },
    ...
    const all = announcements
      .map(a =>
        a.data.map(announcement => ({
          ...
          subText: {
            value: announcement.date ? Helpers.format(announcement.date, 'EEEE, MMMM d, yyyy') : '',
          },
...
const gradesSearchItems = selector<SearchItem[]>({
  ...
  get: ({ get }) => {
    ...
    return grades.data.map(({ id, attributes }) => ({
      ...
      link: {
        to: `${Routes.Routes()['past courses'].fullPath}?c=${attributes.courseSubject}+${attributes.courseNumber}`,
      },
      attr: {
        grades: { ...attributes },
      },
...
const pageToRoute = (p: string, dashboard?: string) => {
  ...
  if (Routes.Routes(dashboard)[page]) {
    return Routes.Routes(dashboard)[page].fullPath;
  } else {
    ...
    return notifications.data.map(notification => ({
      ...
      subText: {
        value: notification.deliveredAt
          ? `Received ${Helpers.format(notification.deliveredAt, "EEEE, MMMM d'th at ' h:mm a")}`
          : '',
      },
      link: { modal: true },
      attr: {
...
const plannerItemSearchItems = selector<SearchItem[]>({
  ...
  get: ({ get }) => {
    ...
    return plannerItems.data.map(plannerItem => {
      const matched = Course.matchedCourseContext(courses.data, plannerItem.context_name);
      const subTextItems: string[] = [];
      if (matched) {
        subTextItems.push(`${matched.courseSubject} ${matched.courseNumber}`);
      }
      const date = Course.plannerItemDate(plannerItem.context_type, plannerItem.plannable_date);
      if (date) {
      ...
      return {
        ...
        link: { href: Canvas.canvasUrl(plannerItem.html_url ?? '') },
        attr: {
          plannerItem,
        },
      };
